...
void LibeventServer::RegisterReadingCallback(pCallbackHandleReading pFn, void* pUser)
{
    if (pFn != nullptr)
        m_cbReading = pFn;
    if (pUser)
...
void LibeventServer::PushMsg(evutil_socket_t fd, const std::string& strData)
{
    if (m_cbReading)
        m_cbReading(fd, strData, m_pUser);
    else
    {
        // do something
    }
...
void LibeventServer::read_cb(bufferevent *pEvtBuffer, void *arg)
{
    ...
    LibeventServer::Instance().PushMsg(fd, msg);
}
...
{
    // as the client connect success, it'll trigger
