...
const char* ConfigurationTypeHelper::convertToString(ConfigurationType configurationType) {
  ...
  // modified:20200114  support for time rolling
  if (configurationType == ConfigurationType::LogFileRollingTime) return "LOG_FILE_ROLLING_TIME";
  return "UNKNOWN";
}
...
static struct ConfigurationStringToTypeItem configStringToTypeMap[] = {
  ...
  // modified: 20200114 support time-rolling of file
  { "log_file_rolling_time", ConfigurationType::LogFileRollingTime },
};
...
void Configurations::setToDefault(void) {
  ...
  // modified:20200116  support for day rolling
  setGlobally(ConfigurationType::LogFileRollingTime, std::string("day"), true);
...
void Configurations::setRemainingToDefault(void) {
  ...
  // modified:20200116  support for day-rolling
  unsafeSetIfNotExist(Level::Global, ConfigurationType::LogFileRollingTime, std::string("day"));
...
struct ::tm DateTime::getCurrentDateTime() {
    struct timeval currTime;
    gettimeofday(&currTime);
    struct ::tm timeInfo;
...
std::size_t TypedConfigurations::dateTimeFormatForRollingLog(Level level)
{
...
void TypedConfigurations::build(Configurations* configurations) {
  ...
  //    conf != withFileSizeLimit.end(); ++conf) {
  //    // This is not unsafe as mutex is locked in currect scope
  //    unsafeValidateFileRollingOnTime((*conf)->level(), base::defaultPreRollOutCallback);
  //}
}
...
void TypedConfigurations::insertFile(Level level, const std::string& fullFilename) {
  ...
  // modified:20200118  support for day-rolling
...
// modified:20200116    support for day-rolling
bool TypedConfigurations::unsafeValidateFileRollingOnTime(Level level, const PreRollOutCallback& PreRollOutCallback) {
    base::type::fstream_t* fs = unsafeGetConfigByRef(level, &m_fileStreamMap, "fileStream").get();
    if (fs == nullptr) {
        return true;
    }
...
bool TypedConfigurations::CompareDateTimeForRollingLog(Level level, const std::string& file)
{
...
void LogDispatcher::dispatch(void) {
  ...
  if (ELPP->hasFlag(LoggingFlag::StrictLogFileTimeCheck)) {
      tc->validateFileRollingOnTime(m_logMessage->level(), ELPP->preRollOutCallback());
  }
