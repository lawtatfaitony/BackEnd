...
// 只处理Unicode 在 [0X4E00, 0X9FA5] 间的汉子, 检查是否在此区间
bool Hanz2Piny::IsUnicode(const Unicode unicode)
{
    return((begin_hanzi_unicode_ <= unicode) && (unicode <= end_hanzi_unicode_));
}

// 基于 unicode 查找对应汉字的拼音，若为多音字则可能返回多个拼音
std::vector<std::string> Hanz2Piny::GetPinyByUnicode(const Unicode hanzi_unicode, const bool with_tone)
{
    if (!IsUnicode(hanzi_unicode)) {
        //std::cout << "Is not hanzi unicode!" << std::endl;
        return(std::vector<std::string>());
    }
    //std::cout << "Is hanzi unicode!" << std::endl;
    ...
    std::vector<std::string> polyphone_list_with_tone;
    ...
    // 不带声调
    std::vector<std::string> polyphone_list_without_tone;
...
bool Hanz2Piny::IsUtf8 (const std::string& s)
{
    return(utf8::is_valid(s.cbegin(), s.cend()));
}

std::string
Hanz2Piny::GetPinyByUtf8(const std::string& s, const bool with_tone)
{
    std::string pinyin_list_list;
    if (!IsUtf8(s)) {
    ...
    for (auto iter = s.cbegin(); iter != s.cend(); NULL) {
        ...
        std::vector<std::string> pinyin_list = GetPinyByUnicode(unicode, with_tone);
        pinyin_list_list += pinyin_list[0];
    }
...
// 该汉字具体转换的拼音列表（多音字）。
// 非汉字不作任何修改，原样返回。
std::vector<std::pair<bool, std::vector<std::string>>>
Hanz2Piny::GetPinyListByUtf8( const std::string& s,
    const bool with_tone,
    const bool replace_unknown,
    const std::string& replace_unknown_with)
{
    std::vector<std::pair<bool, std::vector<std::string>>> pinyin_list_list;
    if (!IsUtf8(s)) {
        //std::cout << "Is not utf8!" << std::endl;
        static const auto null_pair = std::make_pair(false, std::vector<std::string>());
    ...
    for (auto iter = s.cbegin(); iter != s.cend(); NULL) {
        ...
        pinyin_list = GetPinyByUnicode(unicode, with_tone);
...
// 检查指定文件是否为 UTF-8 编码
bool
Hanz2Piny::IsUtf8File(const std::string& file_path)
{
    ...
    return(utf8::is_valid(std::istream_iterator<char>(ifs), std::istream_iterator<char>()));
}

// 检查指定字符序列是否以 BOMC 打头
bool Hanz2Piny::IsStartWiithbom (const std::string& s)
{
